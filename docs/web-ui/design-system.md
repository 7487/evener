...
Editable text facts render as prefilled form fields in the sheet itself, committed by a
dirty-gated footer Save rather than a second dialog, and renaming is editing the name field;
`panes/settings/sections/marketplacesPlugins/MarketplaceSheet.tsx` — Name plus the same
three-option source picker the add form offers — is the reference implementation.
